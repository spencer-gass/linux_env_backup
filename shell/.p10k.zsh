# Generated by Powerlevel10k configuration wizard on 2024-05-28 at 17:37 EDT.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      #(( $#branch > 32 )) && branch[3,-11]=".."  # <-- this line
      # If it's my branch shorten to sg/branch
      if [[ $branch =~ ^sgass/[^/]+/[^/]+/[^/]+$ ]]; then
        branch="sg/${branch##*/}"
      # if it's someone else's branch shorten to initilas/branch
      elif [[ $branch =~ ^[^/]+/[^/]+/[^/]+/[^/]+$ ]]; then
        branch="${branch:0:2}/${branch##*/}"
      elif [[ ${#branch} -gt 32 ]]; then
        branch="${branch:0:5}..${branch: -16}"
      fi
