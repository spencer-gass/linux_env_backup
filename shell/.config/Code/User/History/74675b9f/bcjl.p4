...
// typedef bit<20*NUM_LABELS>  MplsLabelStackVec;
...
control FRRdplaneIngress(inout headers hdr,
    ...
    *************************************************************************/
    ...
    action ler_push (MplsLabelStackVec out_stack, MplsLabelValidVec lable_valid, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        /* Ethernet */
        hdr.eth.src = src_addr;
        hdr.eth.dst = dst_addr;
        hdr.eth.type = TYPE_MPLS;
        ...
        // hdr.mpls[0].label = out_stack[19:0]; // NOTE(jjardim): single or label stack?
        // hdr.mpls[0].bos = 0;
        // hdr.mpls[0].ttl = 64;
        meta.update_mpls = 1;
    ...
    *************************************************************************/
    ...
    mpls_t[NUM_LABELS] old_mpls_stack = hdr.mpls;
    mpls_t[NUM_LABELS] new_mpls_stack;
    bit<3> label_index;
    ...
    apply {
        ...
        if (meta.update_mpls == 1) {
            label_index = 0;
            if (meta.newMplsValid & 0x01 == 0x01) {
                hdr.mpls[label_index].label = meta.newMplsStack[31:12];
                hdr.mpls[label_index].exp   = meta.newMplsStack[11:9];
                hdr.mpls[label_index].bos   = meta.newMplsStack[8:8];
                hdr.mpls[label_index].ttl   = meta.newMplsStack[7:0];
                label_index = label_index + 1;
            }
            if (meta.newMplsValid & 0x02 == 0x02) {
                hdr.mpls[label_index].label = meta.newMplsStack[63:44];
                hdr.mpls[label_index].exp   = meta.newMplsStack[43:41];
                hdr.mpls[label_index].bos   = meta.newMplsStack[40:40];
                hdr.mpls[label_index].ttl   = meta.newMplsStack[39:32];
                label_index = label_index + 1;
            }
        }

        // No if statements in action blocks -> put actions params in user metadata and process in the control apply block
        // No loops -> do a sequence of if statements
        // No variable array indexing -> no solution in p4. try user extern
