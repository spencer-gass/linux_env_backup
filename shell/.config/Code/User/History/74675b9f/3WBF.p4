    ...
    apply {
        ...
        if (meta.update_mpls == 1) {
            ...
            if (meta.newMplsValid & 0x02 == 0x02) {
                hdr.mpls[label_index].label = meta.newMplsStack[31+32:12+32];
                hdr.mpls[label_index].exp   = meta.newMplsStack[11+32:9+32];
                hdr.mpls[label_index].bos   = meta.newMplsStack[8+32:8+32];
                hdr.mpls[label_index].ttl   = meta.newMplsStack[7+32:0+32];
                label_index = label_index + 1;
            }
        }
