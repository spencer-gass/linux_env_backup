...
class P4RouterAvmm(AvmmCommonCtrl):
    ...
    def print_queue_drop_counters(self, queue, print_zeros=False):
        ...
        """
        ing_policer_drops = self.read_and_clear_drop_counter(queue,self.ING_POLICER_DROPS)
        queue_full_drops = self.read_and_clear_drop_counter(queue,self.QUEUE_FULL_DROPS)
        malloc_drops = self.read_and_clear_drop_counter(queue,self.MALLOC_DROP)
        mem_full_drops = self.read_and_clear_drop_counter(queue,self.MEM_FULL_DROPS)
        b2b_drops = self.read_and_clear_drop_counter(queue,self.B2B_DROPS)
