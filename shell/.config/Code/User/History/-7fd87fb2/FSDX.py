...
class P4RouterAvmmAddrs(IntEnum):
    ...
    ADDR_ING_CNTRS_RD_DATA0 = REG_WIDTH_BYTES * 24
    ADDR_ING_CNTRS_RD_DATA1 = REG_WIDTH_BYTES * 25
    ADDR_EGR_CNTRS_SAMPLE_CON = REG_WIDTH_BYTES * 26
    ADDR_EGR_CNTRS_RD_SEL = REG_WIDTH_BYTES * 27
    ADDR_EGR_CNTRS_RD_DATA0 = REG_WIDTH_BYTES * 28
    ADDR_EGR_CNTRS_RD_DATA1 = REG_WIDTH_BYTES * 29
    ADDR_ING_POLICER_ENABLE = REG_WIDTH_BYTES * 30
    ADDR_QSYS_TABLE_CONFIG = REG_WIDTH_BYTES * 31
    ADDR_QSYS_CONFIG_WDATA = REG_WIDTH_BYTES * 32
    ADDR_QSYS_CONFIG_RDATA = REG_WIDTH_BYTES * 33
    ADDR_QSYS_CNTR_CON = REG_WIDTH_BYTES * 34
    ADDR_QSYS_CNTR_RDATA = REG_WIDTH_BYTES * 35
    ADDR_DBG_CNTR_CON = REG_WIDTH_BYTES * 36
    ADDR_DBG_CNTR_RDATA0 = REG_WIDTH_BYTES * 37
    ADDR_DBG_CNTR_RDATA1 = REG_WIDTH_BYTES * 38

    PACKET_COUNT_INDEX = 0
    BYTE_COUNT_INDEX = 1
    ERR_COUNT_INDEX = 2
    ING_ASYNC_FIFO_OVERFLOW_INDEX = 3
    ING_BUF_OVERFLOW_INDEX = 4
    EGR_BUF_OVERFLOW_INDEX = 3
    NUM_ING_COUTNERS = 5
    NUM_EGR_COUNTERS = 4
...
class P4RouterAvmm(AvmmCommonCtrl):
    ...
    ing_cntrs_rd_data0 = MMIRODesc(
            ...
            addr=P4RouterAvmmAddrs.ADDR_ING_CNTRS_RD_DATA0,
            msb=31,
            lsb=0
    )
    ...
    ing_cntrs_rd_data1 = MMIRODesc(
            ...
            addr=P4RouterAvmmAddrs.ADDR_ING_CNTRS_RD_DATA1,
            msb=31,
            lsb=0
    )
    ...
    egr_cntrs_rd_data0 = MMIRODesc(
            ...
            addr=P4RouterAvmmAddrs.ADDR_EGR_CNTRS_RD_DATA0,
            msb=31,
            lsb=0
    )
    ...
    egr_cntrs_rd_data1 = MMIRODesc(
            ...
            addr=P4RouterAvmmAddrs.ADDR_EGR_CNTRS_RD_DATA1,
            msb=31,
            lsb=0
    )
    ...
    dbg_cntr_rd_data0 = MMIRODesc(
            name="dbg_cntr_rd_data0",
            addr=P4RouterAvmmAddrs.ADDR_DBG_CNTR_RDATA0,
            msb=31,
            lsb=0
    )
    dbg_cntr_rd_data1 = MMIRODesc(
            name="dbg_cntr_rd_data1",
            addr=P4RouterAvmmAddrs.ADDR_DBG_CNTR_RDATA1,
            msb=31,
            lsb=0
    )

    dbg_cntr_clear = MMIDescBit(
            name="dbg_cntr_clear",
            addr=P4RouterAvmmAddrs.ADDR_DBG_CNTR_CON,
            bit=0
    )

    dbg_cntr_sel = MMIDesc(
            name="dbg_cntr_sel",
            addr=P4RouterAvmmAddrs.ADDR_DBG_CNTR_CON,
            msb=7,
            lsb=4
    ...
    def wr_ing_policer_cir_table(self, addr, data):
        return self._wr_qsys_table(P4RouterAvmmAddrs.ING_POLICER_CIR_TABLE, addr, data)

    def wr_ing_policer_cbs_table(self, addr, data):
        return self._wr_qsys_table(P4RouterAvmmAddrs.ING_POLICER_CBS_TABLE, addr, data)

    def wr_cong_man_drop_thresh_table(self, addr, data):
        return self._wr_qsys_table(P4RouterAvmmAddrs.CONG_MAN_DROP_THRESH_TABLE, addr, data)
    ...
    def rd_ing_policer_cir_table(self, addr):
        return self._rd_qsys_table(P4RouterAvmmAddrs.ING_POLICER_CIR_TABLE, addr)

    def rd_ing_policer_cbs_table(self, addr):
        return self._rd_qsys_table(P4RouterAvmmAddrs.ING_POLICER_CBS_TABLE, addr)

    def rd_cong_man_drop_thresh_table(self, addr):
        return self._rd_qsys_table(P4RouterAvmmAddrs.CONG_MAN_DROP_THRESH_TABLE, addr)
    ...
    def clear_debug_couters(self):
        self.dbg_cntr_clear = 1
        self.dbg_cntr_clear = 0

    def get_dbg_cntr(self, index):
        self.dbg_cntr_sel = index
        return (self.dbg_cntr_rd_data1 << 32) + self.dbg_cntr_rd_data0

    def print_and_clear_debug_counters(self):
        """
        Args:
        ...
        print("Ingress Bus   : {}".format(self.get_dbg_cntr(P4RouterAvmmAddrs.ING_BUS_DBG_CNT)))
        print("Policer       : {}".format(self.get_dbg_cntr(P4RouterAvmmAddrs.POLICER_DBG_CNT)))
        print("Enqueue       : {}".format(self.get_dbg_cntr(P4RouterAvmmAddrs.ENQUEUE_DBG_CNT)))
        print("Dequeue       : {}".format(self.get_dbg_cntr(P4RouterAvmmAddrs.DEQUEUE_DBG_CNT)))
        print('Egress Ports  : {}'.format(egr_pkt_cnt))

        self.clear_debug_couters()
