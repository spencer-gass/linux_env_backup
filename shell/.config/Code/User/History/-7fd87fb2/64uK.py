...
class P4RouterAvmm(AvmmCommonCtrl):
    ...
    clock_period_ps = MMIRODesc(name="clock_period_ps", addr=P4RouterAvmmAddrs.ADDR_PARAMS1, msb=31, lsb=16)
    ...
    qsys_table_wr_err = MMIRODescBit(name="qsys_table_wr_err", addr=P4RouterAvmmAddrs.ADDR_QSYS_TABLE_CONFIG, bit=29)
    qsys_table_rd_err = MMIRODescBit(name="qsys_table_rd_err", addr=P4RouterAvmmAddrs.ADDR_QSYS_TABLE_CONFIG, bit=28)
    qsys_table_wr_busy = MMIRODescBit(name="qsys_table_wr_busy", addr=P4RouterAvmmAddrs.ADDR_QSYS_TABLE_CONFIG, bit=27)
    qsys_table_rd_busy = MMIRODescBit(name="qsys_table_rd_busy", addr=P4RouterAvmmAddrs.ADDR_QSYS_TABLE_CONFIG, bit=26)
    ...
    def _wr_qsys_table(self, table, addr, data):
        ...
        while (self.qsys_table_wr_busy and cnt < 10):
            cnt += 1
            sleep(0.01)
        if (self.qsys_table_wr_busy):
            print("Queue System Table Write Timed Out.")
            return 1
    ...
    def wr_ing_policer_cir_table(self, addr, data):
        self._wr_qsys_table(self.ING_POLICER_CIR_TABLE, addr, data)

    def wr_ing_policer_cbs_table(self, addr, data):
        self._wr_qsys_table(self.ING_POLICER_CBS_TABLE, addr, data)

    def wr_cong_man_drop_thresh_table(self, addr, data):
        self._wr_qsys_table(self.CONG_MAN_DROP_THRESH_TABLE, addr, data)
