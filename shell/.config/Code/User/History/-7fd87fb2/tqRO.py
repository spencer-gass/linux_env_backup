...
class P4RouterAvmm(AvmmCommonCtrl):
    ...
    def drop_counter_op(self, queue, type, op):
        self.qsys_cntr_op_req = 0
        self.qsys_cntr_op_code = op
        self.qsys_cntr_type_sel = type
        ...
        if self.qsys_cntr_op_err:
            raise Exception("Queue System Counter Op Error. Probably invalid queue or counter type.")

        return self.qsys_cntr_rd_data

    def read_drop_counter(self, queue, type):
        return self.drop_counter_op(queue, type, READ)

    def read_and_clear_drop_counter(self, queue, type):
        return self.drop_counter_op(queue, type, READ_AND_CLEAR)

    def clear_all_drop_counters(self):
        return self.drop_counter_op(0, 0, CLEAR_ALL)
    ...
    def print_queue_drop_counters(self, queue, print_zeros=False):
        ...
        if ing_policer_drops or queue_full_drops or malloc_drops or mem_full_drops or b2b_drops or print_zeros:
