...
key_bits, action_id_bits, action_param_bits, num_rows
...
class VNP4Avmm(AvmmCommonCtrl):
    ...
    def __init__(self, sdr_host, offset):
        ...
        self.key_bits = VNP4AvmmAddrs.CAM_KEY_BITS
        self.action_id_bits = VNP4AvmmAddrs.CAM_ACTION_ID_BITS
        self.action_param_bits = VNP4AvmmAddrs.CAM_ACTION_PARAM_BITS
        self.num_rows = VNP4AvmmAddrs.CAM_NUM_ROWS
    ...
    def _set_cam_offset(self, table_id, offset):
        offset = VNP4AvmmAddrs.REG_WIDTH_BYTES * offset
        self.cam_offsets[table_id] = offset
        cam_ctrl[table_id].addr = VNP4AvmmAddrs.CAM_CTRL + offset
        rd_flag[table_id].addr = VNP4AvmmAddrs.CAM_CTRL + offset
        wr_flag[table_id].addr = VNP4AvmmAddrs.CAM_CTRL + offset
        reset[table_id].addr = VNP4AvmmAddrs.CAM_CTRL + offset
        entry_in_use[table_id].addr = VNP4AvmmAddrs.CAM_CTRL + offset

        entry_id[table_id] = VNP4AvmmAddrs.CAM_ENTRY_ID + offset
        emulation_mode[table_id] = VNP4AvmmAddrs.CAM_EMULATION_MODE + offset
        lookup_count[table_id] = VNP4AvmmAddrs.CAM_LOOKUP_COUNT + offset
        hit_count[table_id] = VNP4AvmmAddrs.CAM_HIT_COUNT + offset
        miss_count[table_id] = VNP4AvmmAddrs.CAM_MISS_COUNT + offset
    ...
    def set_cam_params(self, table_id, offset, key_bits, action_id_bits, action_param_bits, num_rows):
        self._validate_table_id(table_id)
        self._set_cam_offset(tale_id, offset)
        self.key_bits[table_id] = key_bits
        self.action_id_bits[table_id] = action_id_bits
        self.action_param_bits[table_id] = action_param_bits
        self.num_rows[table_id] = num_rows

    def get_cam_params(self, table_id):
        self._validate_table_id(table_id)
        return [
            self.cam_offsets[table_id],
            self.key_bits[table_id],
            self.action_id_bits[table_id],
            self.action_param_bits[table_id],
            self.num_rows[table_id]
        ]
    ...
    def _read_table_row(self, table_id row_id, timeout_s=0.01):
        ...
        # Read back key
        words_per_key = (self.key_bits[table_id] + 31) // 32
        key = self.sdr.mmi_mw_read(
                self.cam_offsets[table_id] + VNP4AvmmAddrs.CAM_DATA0,
                words_per_key,
        ...
        value = self.sdr.mmi_mw_read(
                self.cam_offsets[table_id] + VNP4AvmmAddrs.CAM_DATA0 + self.value_offset,
