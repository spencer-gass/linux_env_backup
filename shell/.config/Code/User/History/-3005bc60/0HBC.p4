...
/* USE_BM should be defined/enabled when testing with a BMv2 target
 * BM mode uses Standard Metadata vs. User-defined Metadata.
 * Disable if/when testing in production (e.g., Xilinx Vitis Networking P4).
 */
...
const bit<4> DONT_DROP = 0x0;
const bit<4> INTF_MAP_DROP = 0x1;
const bit<4> LFIB_DROP = 0x2;
const bit<4> IPV4_FIB_INGRESS_DROP = 0x3;
const bit<4> CMP_IPV4_FIB_DROP = 0x4;
const bit<4> CMP_MAC_FIB_DROP = 0x5;
const bit<4> VLAN_MAP_DROP = 0x6;
const bit<4> IPV4_CHK_SUM_DROP = 0x7;

/* Field sizes */
const bit<14> VLAN_TAG_BYTES = 4;
const bit<14> MPLS_LABEL_BYTES = 4;
...
struct user_metadata {
    PortId ingress_port;    // Ingress port
    PortId egress_port;     // Egress port
    ...
    bit <4> drop_reason;
    EtherType ether_type;     // Payload protocol type
    MplsLabel mpls_label;    // MPLS label value
}
...
control FRRdplaneIngress(inout headers hdr,
    ...
    *************************************************************************/
    ...
    action strip_dot1q(PortId port) {
        ...
        /* Update byte length */
        meta.byte_length = meta.byte_length - VLAN_TAG_BYTES;
    ...
    *************************************************************************/
    ...
    action ler_single_push (MplsLabel vpn_label, MplsLabel transport_label, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        ...
        /* Update byte length */
        meta.byte_length = meta.byte_length + MPLS_LABEL_BYTES;
    ...
    action ler_push (MplsLabel vpn_label, MplsLabel transport_label, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        ...
        /* Update byte length */
        meta.byte_length = meta.byte_length + 2*MPLS_LABEL_BYTES;
    ...
    *************************************************************************/
    ...
    apply {
        meta.drop_reason = DONT_DROP;
        meta.ether_type = hdr.eth.type;
        meta.mpls_label = hdr.mpls[0].label;
        /* Initialize user-defined metadata */
        intf_map.apply();
        if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
            meta.drop_reason = INTF_MAP_DROP;
        }
        ...
        if (meta.ingress_port != CPU_PORT) { // non-CPU originated
            ...
            if (hdr.eth.type == TYPE_MPLS) {
                lfib.apply();
                if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                    meta.drop_reason = LFIB_DROP;
                }
                if (meta.bos == 1) {
                    ipv4_fib_ingress.apply();
                    // if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                    //     meta.drop_reason = IPV4_FIB_INGRESS_DROP;
                    // }
                }
            } else if (hdr.eth.type == TYPE_IPV4) {
                if (!cmp_ipv4_fib.apply().hit) {
                    if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                        meta.drop_reason = CMP_IPV4_FIB_DROP;
                    }
                    ipv4_fib_ingress.apply();
                    // if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                    //     meta.drop_reason = IPV4_FIB_INGRESS_DROP;
                ...
                cmp_mac_fib.apply();
                if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                    meta.drop_reason = CMP_MAC_FIB_DROP;
                }
            }
            ...
            vlan_map.apply();
            if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                meta.drop_reason = VLAN_MAP_DROP;
            }
        }
        ...
        if (hdr.ipv4.isValid()) {
            ...
            if (checksum_valid == 0) {
                drop();
                if (meta.drop_reason == DONT_DROP && standard_metadata.drop == 1) {
                    meta.drop_reason = IPV4_CHK_SUM_DROP;
                }
            }
