...
control MyProcessing(inout headers hdr,
    ...
    action IPv4ChkVerify() {
        ...
        chksum = chksum + hdr.ipv4.version << 12 | hdr.ipv4.hdr_len << 8 | hdr.ipv4.tos;
        chksum = chksum + hdr.ipv4.length;
        chksum = chksum + hdr.ipv4.id;
        chksum = chksum + (hdr.ipv4.flags << 13) | hdr.ipv4.offset;
        chksum = chksum + hdr.ipv4.ttl << 8 | hdr.ipv4.protocol;
        chksum = chksum + hdr.ipv4.src[31:16];
        chksum = chksum + hdr.ipv4.src[15:0];
        chksum = chksum + hdr.ipv4.dst[31:16];
        chksum = chksum + hdr.ipv4.dst[15:0];
        ...
        if (chksum[15:0] != hdr.ipv4.hdrChecksum) {
            dropPacket();
        }
    }
