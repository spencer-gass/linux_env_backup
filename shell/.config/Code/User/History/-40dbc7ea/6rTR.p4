...
control MyProcessing(inout headers hdr,
    ...
    action IPv4ChkVerify() {
        ...
        chksum = chksum + ipv4.version << 12 | ipv4.hdr_len << 8 | ipv4.tos;
        chksum = chksum + ipv4.length;
        chksum = chksum + ipv4.id;
        chksum = chksum + (ipv4.flags << 13) | ipv4.offset;
        chksum = chksum + ipv4.ttl << 8 | ipv4.protocol;
        chksum = chksum + ipv4.src[31:16];
        chksum = chksum + ipv4.src[15:0];
        chksum = chksum + ipv4.dst[31:16];
        chksum = chksum + ipv4.dst[15:0];
        ...
        if (chksum[15:0] != ipv4.hdrChecksum) {
            dropPacket();
        }
    }
