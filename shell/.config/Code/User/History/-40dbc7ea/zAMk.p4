...
control MyProcessing(inout headers hdr,
    ...
    action IPv4ChkVerify() {
        ...
        chksum = chksum + (4'0x0 ++ hdr.ipv4.version ++ hdr.ipv4.hdr_len ++ hdr.ipv4.tos);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.length);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.id);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.flags ++ hdr.ipv4.offset);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.ttl ++ hdr.ipv4.protocol);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.src[31:16]);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.src[15:0]);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.dst[31:16]);
        chksum = chksum + (4'0x0 ++ hdr.ipv4.dst[15:0]);
