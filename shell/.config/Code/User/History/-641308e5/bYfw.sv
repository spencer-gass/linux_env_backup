...
#(
    ...
    Clock_int unused_clk_ifc        [-1:0] ();
    Reset_int unused_sreset_ifc [-1:0] ();

    AXIS_int #(
        .DATA_BYTES(8 * PPL_NUM_LANES)
    ) unused_ppl_ins [0:0][-1:0][0:0] (
        .clk     (  unused_clk_ifc.clk      ),
        .sresetn (  unused_sreset_ifc.reset )
    );

    AXIS_int #(
        .DATA_BYTES(8 * PPL_NUM_LANES)
    ) unused_ppl_outs [0:0][-1:0][0:0] (
        .clk     (  unused_clk_ifc.clk      ),
        .sresetn (  unused_sreset_ifc.reset )
    );
    ...
    ethernet_to_axis #(
        ...
        .clk_ifc_mac_gmiis       ( unused_clk_ifc        ),
        .sreset_ifc_mac_gmiis    ( unused_sreset_ifc     ),
        .mac_gmiis               ( unused_phy_gmiis      ),
        .sreset_ifc_mac_xgmiis   ( unused_clk_ifc        ),
    ...
    generate
        ...
        for (genvar k = PHY_LOOPBACK_OFFSET; k < PHY_LOOPBACK_OFFSET + NUM_PHY_LOOPBACKS; k++) begin : gen_phy_loopback
