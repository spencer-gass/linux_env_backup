...
// not functionally correct but needed something that would pass Xilinx p4 compiler
header ip_version_t {
    bit<8>    version;
}
...
parser FRRdplaneParser(packet_in packet,
    ...
    state parse_mpls_payload {
        packet.extract(hdr.ip_version);
        transition select(hdr.ip_version.version) {
             4: parse_ipv4;
             6: parse_ipv6;
        }
    }

    //sg// state parse_mpls_payload {
    //sg//     transition select(packet.lookahead<ipv4_t>().version) {
    //sg//         4: parse_ipv4;
    //sg//         6: parse_ipv6;
    //sg//         default: accept;
    //sg//     }
...
control FRRdplaneIngress(inout headers hdr,
    ...
    action drop() {
        //sg//ark_to_drop(standard_metadata);
...
XilinxPipeline(
FRRdplaneParser(),
//sg//FRRdplaneVerifyChecksum(),
FRRdplaneIngress(),
//sg//FRRdplaneEgress(),
//sg//FRRdplaneComputeChecksum(),
FRRdplaneDeparser()
) main;
