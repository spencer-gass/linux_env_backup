...
/**
 * This module handles GMII/XGMII translation to AXI streams both ways.
 */
...
#(
    ...
    localparam int                      NUM_AXIS            = NUM_PHY_PPLS
                                                            + NUM_PHY_XGMIIS
                                                            + NUM_PHY_GMIIS
                                                            + NUM_MAC_XGMIIS
                                                            + NUM_MAC_GMIIS,
    ...
    Clock_int.Input     clk_ifc_phy_xgmiis      [NUM_PHY_XGMIIS-1:0],
    Reset_int.ResetIn   sreset_ifc_phy_xgmiis   [NUM_PHY_XGMIIS-1:0],
    ...
    localparam int  NUM_PHYS = NUM_PHY_GMIIS + NUM_PHY_XGMIIS + NUM_PHY_PPLS;
    localparam int  NUM_MACS = NUM_MAC_GMIIS + NUM_MAC_XGMIIS;
    ...
    generate
        for (genvar axis_idx = 0; axis_idx < NUM_AXIS; axis_idx++) begin : gen_interfaces
            ...
            if ( axis_idx >=  MAC_GMII_OFFSET && axis_idx < MAC_GMII_OFFSET + NUM_MAC_GMIIS ) begin : gen_mac_gmii
                ...
                eth_mac_1g_fifo #(
                    ...
                    .tx_error_underflow (mac_fifo_tx_error_underflow[axis_idx]),
                    .tx_fifo_overflow   (mac_fifo_tx_fifo_overflow  [axis_idx]),
                    .tx_fifo_bad_frame  (mac_fifo_tx_fifo_bad_frame [axis_idx]),
                    .tx_fifo_good_frame (mac_fifo_tx_fifo_good_frame[axis_idx]),
                    .rx_error_bad_frame (mac_fifo_rx_error_bad_frame[axis_idx]),
                    .rx_error_bad_fcs   (mac_fifo_rx_error_bad_fcs  [axis_idx]),
                    .rx_fifo_overflow   (mac_fifo_rx_fifo_overflow  [axis_idx]),
                    .rx_fifo_bad_frame  (mac_fifo_rx_fifo_bad_frame [axis_idx]),
                    .rx_fifo_good_frame (mac_fifo_rx_fifo_good_frame[axis_idx])
                );
                ...
                if (MAC_GMII_ILA_MASK[mac_gmii_idx]) begin : gen_mac_ila
                    ila_debug gmii_ila (
                        ...
                        .probe9( {  mac_fifo_tx_error_underflow[axis_idx], mac_fifo_tx_fifo_overflow[axis_idx], mac_fifo_tx_fifo_bad_frame[axis_idx],
                                    mac_fifo_tx_fifo_good_frame[axis_idx], mac_fifo_rx_error_bad_frame[axis_idx], mac_fifo_rx_error_bad_fcs[axis_idx],
                                    mac_fifo_rx_fifo_overflow[axis_idx], mac_fifo_rx_fifo_bad_frame[axis_idx], mac_fifo_rx_fifo_good_frame[axis_idx]
                                  }  ),
                        .probe10( {mac_phy_sel[axis_idx]} ),
                        .probe11( {mac_phy_sel_invalid[axis_idx]} ),
                        .probe12( {phy_mac_sel} ),
                        .probe13( '0 ),
                        .probe14( '0 ),
                        .probe15( '0 )
            ...
            end else if ( axis_idx >=  MAC_XGMII_OFFSET && axis_idx < MAC_XGMII_OFFSET + NUM_MAC_XGMIIS ) begin : gen_xgmii
        ...
        for (genvar axis_idx = 0; axis_idx < NUM_PHYS; axis_idx++) begin : gen_phy_interface

            if ( axis_idx >=  PHY_GMII_OFFSET && axis_idx < PHY_GMII_OFFSET + NUM_PHY_GMIIS ) begin : gen_gmii
                ...
                eth_mac_1g_fifo #(
                    ...
                    .tx_axis_tdata      ( int_phy_axis_in[axis_idx].tdata  ),
                    .tx_axis_tkeep      ( '1                ),
                    .tx_axis_tvalid     ( int_phy_axis_in[axis_idx].tvalid ),
                    .tx_axis_tready     ( int_phy_axis_in[axis_idx].tready ),
                    .tx_axis_tlast      ( int_phy_axis_in[axis_idx].tlast  ),
                    .tx_axis_tuser      ( int_phy_axis_in[axis_idx].tuser  ),

                    .rx_axis_tdata      ( int_phy_axis_out[axis_idx].tdata  ),
                    .rx_axis_tkeep      ( int_phy_axis_out[axis_idx].tkeep  ),
                    .rx_axis_tvalid     ( int_phy_axis_out[axis_idx].tvalid ),
                    .rx_axis_tready     ( int_phy_axis_out[axis_idx].tready ),
                    .rx_axis_tlast      ( int_phy_axis_out[axis_idx].tlast  ),
                    .rx_axis_tuser      ( int_phy_axis_out[axis_idx].tuser  ),
                    ...
                    .tx_error_underflow (phy_fifo_tx_error_underflow[axis_idx]),
                    .tx_fifo_overflow   (phy_fifo_tx_fifo_overflow  [axis_idx]),
                    .tx_fifo_bad_frame  (phy_fifo_tx_fifo_bad_frame [axis_idx]),
                    .tx_fifo_good_frame (phy_fifo_tx_fifo_good_frame[axis_idx]),
                    .rx_error_bad_frame (phy_fifo_rx_error_bad_frame[axis_idx]),
                    .rx_error_bad_fcs   (phy_fifo_rx_error_bad_fcs  [axis_idx]),
                    .rx_fifo_overflow   (phy_fifo_rx_fifo_overflow  [axis_idx]),
                    .rx_fifo_bad_frame  (phy_fifo_rx_fifo_bad_frame [axis_idx]),
                    .rx_fifo_good_frame (phy_fifo_rx_fifo_good_frame[axis_idx])
                );

                // Unused
                assign int_phy_axis_out[axis_idx].tstrb = '1;
                assign int_phy_axis_out[axis_idx].tid   = '0;
                assign int_phy_axis_out[axis_idx].tdest = '0;

                if (PHY_GMII_ILA_MASK[phy_gmiii_idx]) begin : gen_phy_ila
                    ila_debug gmii_ila (
                        .clk    ( clk_ifc_axis.clk  ),
                        .probe0 ( {int_phy_axis_in[axis_idx].tdata,   int_phy_axis_in[axis_idx].tkeep} ),
                        .probe1 ( {int_phy_axis_in[axis_idx].tvalid,   int_phy_axis_in[axis_idx].tready} ),
                        .probe2 ( {int_phy_axis_in[axis_idx].tuser,    int_phy_axis_in[axis_idx].tlast}  ),
                        .probe3 ( {int_phy_axis_out[axis_idx].tdata,  int_phy_axis_out[axis_idx].tkeep} ),
                        .probe4 ( {int_phy_axis_out[axis_idx].tvalid, int_phy_axis_out[axis_idx].tready} ),
                        .probe5 ( {int_phy_axis_out[axis_idx].tlast,  int_phy_axis_out[axis_idx].tuser} ),
                        .probe6( {int_axis_sreset } ),
                        .probe7( {phy_gmiis[phy_gmiii_idx].tx_d, phy_gmiis[phy_gmiii_idx].tx_en, phy_gmiis[phy_gmiii_idx].tx_er} ),
                        .probe8( {phy_gmiis[phy_gmiii_idx].rx_d, phy_gmiis[phy_gmiii_idx].rx_dv, phy_gmiis[phy_gmiii_idx].rx_er} ),
                        .probe9( {  phy_fifo_tx_error_underflow[axis_idx], phy_fifo_tx_fifo_overflow[axis_idx], phy_fifo_tx_fifo_bad_frame[axis_idx],
                                    phy_fifo_tx_fifo_good_frame[axis_idx], phy_fifo_rx_error_bad_frame[axis_idx], phy_fifo_rx_error_bad_fcs[axis_idx],
                                    phy_fifo_rx_fifo_overflow[axis_idx], phy_fifo_rx_fifo_bad_frame[axis_idx], phy_fifo_rx_fifo_good_frame[axis_idx]
                                  }  ),
                        .probe10( {phy_mac_sel[axis_idx]} ),
                        .probe11( {phy_mac_sel_invalid[axis_idx]} ),
                        .probe12( {mac_phy_sel} ),
                        .probe13( '0 ),
                        .probe14( '0 ),
                        .probe15( '0 )
            ...
            end else if ( axis_idx >=  PHY_XGMII_OFFSET && axis_idx < PHY_XGMII_OFFSET + NUM_PHY_XGMIIS ) begin : gen_xgmii
                ...
                if (PHY_XGMII_ILA_MASK[phy_xgmiii_idx]) begin : gen_phy_ila
                    ila_debug xgmii_ila (
                        .clk    ( clk_ifc_axis.clk  ),
                        .probe0 ( {int_phy_axis_in[axis_idx].tdata} ),
                        .probe1 ( {int_phy_axis_in[axis_idx].tvalid,   int_phy_axis_in[axis_idx].tready, int_phy_axis_in[axis_idx].tkeep} ),
                        .probe2 ( {int_phy_axis_in[axis_idx].tuser,    int_phy_axis_in[axis_idx].tlast}  ),
                        .probe3 ( {int_phy_axis_out[axis_idx].tdata } ),
                        .probe4 ( {int_phy_axis_out[axis_idx].tvalid, int_phy_axis_out[axis_idx].tready, int_phy_axis_out[axis_idx].tkeep} ),
                        .probe5 ( {int_phy_axis_out[axis_idx].tlast,  int_phy_axis_out[axis_idx].tuser} ),
                        .probe6( {int_axis_sreset } ),
                        ...
                        .probe13( {  phy_fifo_tx_error_underflow[axis_idx], phy_fifo_tx_fifo_overflow[axis_idx], phy_fifo_tx_fifo_bad_frame[axis_idx],
                                    phy_fifo_tx_fifo_good_frame[axis_idx], phy_fifo_rx_error_bad_frame[axis_idx], phy_fifo_rx_error_bad_fcs[axis_idx],
                                    phy_fifo_rx_fifo_overflow[axis_idx], phy_fifo_rx_fifo_bad_frame[axis_idx], phy_fifo_rx_fifo_good_frame[axis_idx]
                                  }  ),
                        .probe14( {phy_mac_sel[axis_idx]} ),
                        .probe15( {phy_mac_sel_invalid[axis_idx]} )
                    );
                end
                ...
                ethernet_ppl_fifo #(
                    ...
                    .eth_sink       (int_phy_axis_in[axis_idx:axis_idx]     ),
                    .eth_src        (int_phy_axis_out[axis_idx:axis_idx]    ),
                    .ppl_axis_tx    (ppl_outs_i),
                    .ppl_axis_rx    (ppl_ins_i )
                );

                assign phy_fifo_tx_error_underflow[axis_idx] = 1'b0;
                assign phy_fifo_tx_fifo_overflow  [axis_idx] = 1'b0;
                assign phy_fifo_tx_fifo_bad_frame [axis_idx] = 1'b0;
                assign phy_fifo_tx_fifo_good_frame[axis_idx] = 1'b0;
                assign phy_fifo_rx_error_bad_frame[axis_idx] = 1'b0;
                assign phy_fifo_rx_error_bad_fcs  [axis_idx] = 1'b0;
                assign phy_fifo_rx_fifo_overflow  [axis_idx] = 1'b0;
                assign phy_fifo_rx_fifo_bad_frame [axis_idx] = 1'b0;
                assign phy_fifo_rx_fifo_good_frame[axis_idx] = 1'b0;
