...
control FRRdplaneIngress(inout headers hdr,
    ...
    *************************************************************************/
    ...
    action ler_push_1 (MplsLabel out_label0, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        /* Ethernet */
        hdr.eth.src = src_addr;
        hdr.eth.dst = dst_addr;
        hdr.eth.type = TYPE_MPLS;
        ...
        hdr.mpls[0].label = out_label0; // NOTE(jjardim): single or label stack?
        hdr.mpls[0].bos = 1;
        hdr.mpls[0].ttl = 64;
    ...
    /* Push label stack (transport and/or VPN) to packet at the edge of the MPLS network */
    action ler_push_2 (MplsLabel out_label0, MplsLabel out_label1, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        /* Ethernet */
        hdr.eth.src = src_addr;
        hdr.eth.dst = dst_addr;
        hdr.eth.type = TYPE_MPLS;
        ...
        hdr.mpls[1].label = out_label1; // NOTE(jjardim): single or label stack?
        hdr.mpls[1].bos = 1;
        hdr.mpls[1].ttl = 64;
    ...
    action lsr_push_1(MplsLabel out_label0, MacAddr src_addr, MacAddr dst_addr, PortId port) {
        /* Ethernet */
        hdr.eth.src = src_addr;
        hdr.eth.dst = dst_addr;
        ...
        // shift existing labels
        hdr.mpls[1] = hdr.mpls[0];
        hdr.mpls[2] = hdr.mpls[1];
        hdr.mpls[3] = hdr.mpls[2];
        // etc
    ...
    /* Push transport label to packet traversing the MPLS network */
    action lsr_push_2(MplsLabel out_label0, MplsLabel out_label1,  MacAddr src_addr, MacAddr dst_addr, PortId port) {
        /* Ethernet */
        hdr.eth.src = src_addr;
        hdr.eth.dst = dst_addr;
        ...
        // shift existing labels
        hdr.mpls[2] = hdr.mpls[0];
        hdr.mpls[3] = hdr.mpls[1];
        hdr.mpls[4] = hdr.mpls[2];
        // etc
        ...
        hdr.mpls[1].label = out_label1; // NOTE(jjardim): single or label stack?
        hdr.mpls[1].bos = 0;
        hdr.mpls[1].ttl = 64; /* TODO(jjardim): do we need to decrement the inner label(s) TTL? */
