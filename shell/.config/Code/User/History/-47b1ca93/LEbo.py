...
from kepler.fpga.sim.axis_dpi_pkt_helpers import compare_output_pcap_to_golden, cleanup
...
INPUT_PATH = "../python/kepler/tests/rtlsim/axis/"
...
def create_testbench(vu, lib, project_path, **kwargs):  # pylint: disable=unused-argument
 ...
 # Library for C sources used with DPI
   ...
   # pylint: disable=invalid-name
    for (EN_AXIS_IN, EN_AXIS_OUT) in [(1, 1)] if LOOPBACK else [(1, 0), (0, 1), (1, 1)]:
        for PARALLEL_HEADER in [0, 1]:
            for MAX_LATENCY in [0, 5]:
                ...
                # deterministic.
                do_post_check = (LOOPBACK == 0) and (EN_AXIS_IN == 1) and (MAX_LATENCY == 0)
                ...
                tb.add_config(
                        ...
                        parameters={
                                'input_path':
                                        str(Path(vunit_util.get_project_path()) / INPUT_PATH) + "/",
                                'EN_AXIS_IN':
                                        EN_AXIS_IN,
                                'EN_AXIS_OUT':
                                        EN_AXIS_OUT,
