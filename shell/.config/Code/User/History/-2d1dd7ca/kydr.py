...
def bytes_to_words(num_bytes):
    return (num_bytes + TinyBcamAvmmAddrs.REG_WIDTH_BYTES - 1) // TinyBcamAvmmAddrs.REG_WIDTH_BYTES
...
class TinyBcamAvmm(AvmmCommonCtrl):
    ...
    def __init__(
        ...
        self.value_byte_offset = 2 * self.bytes_per_key
    ...
    def _foramt_entry(self, key, action_id, action_params):
        ...
        mask = 2**(self.key_bits) - 1
        entry = (value << (2 * self.bytes_per_key * 8)) | (mask << (self.bytes_per_key * 8)) | key
        return entry
    ...
    def read_table_row(self, row_idx, timeout_s=0.01):
        ...
        if not self.entry_in_use:
            print("(I) Entry is not populated.")
            return [0, 0, 0]

        # Read back key
        entry = 0
        for i in range(self.words_per_entry):
            word = self.sdr.mmi_read(
                    self.offset + TinyBcamAvmmAddrs.CAM_DATA0
                    + TinyBcamAvmmAddrs.REG_WIDTH_BYTES * i
            )
            entry = entry | word << (TinyBcamAvmmAddrs.REG_WIDTH_BYTES * 8 * i)
    ...
    def write_table_row(self, row_idx, key, action_id, action_params, timeout_s=0.01):
        ...
        # Set entry data
        entry = self._foramt_entry(key, action_id, action_params)
        for i in range(self.words_per_entry):
            self.sdr.mmi_write(
                    self.offset + TinyBcamAvmmAddrs.CAM_DATA0
                    + TinyBcamAvmmAddrs.REG_WIDTH_BYTES * i,
                    entry & 2**(TinyBcamAvmmAddrs.REG_WIDTH_BYTES * 8) - 1
            )
            entry = entry >> (TinyBcamAvmmAddrs.REG_WIDTH_BYTES * 8)
    ...
    def load_config_from_dict(self, config_dict):
        '''
        Args:
            config_dict (dict): configuration dictionary from yaml parser
        '''
        for row_idx in range(self.num_rows):
            self.clear_table_row(row_idx)
        for row_idx, entry_dict in enumerate(config_dict):
            action_params = cat_fields(entry_dict["action_params"])
    ...
    def load_cam_config_from_yaml(self, fname):
        ...
        Raises:
            ValueError: if table name isn't found in the first level of hierarchy in the yaml file
        '''
        print(f"Loading config for {self.table_name}.")
        with open(fname, 'r', encoding="utf-8") as file:
            cam_config_dict = yaml.safe_load(file)
        if self.table_name not in cam_config_dict.keys():
            raise ValueError(f"{self.table_name} wasn't found in yaml config.")
