...
class TinyBcamAvmm(AvmmCommonCtrl):
    ...
    def _foramt_entry(self, key, action_id, action_params):
        ...
        entry = (value << (2 * self.bytes_per_key * 8)) | key
        return entry
    ...
    def read_table_row(self, row_idx, timeout_s=0.01):
        ...
        entry = 0
        for w in range(self.words_per_entry):
            word = self.sdr.mmi_read(self.offset + TinyBcamAvmmAddrs.CAM_DATA0 + TinyBcamAvmmAddrs.REG_WIDTH_BYTES * w)
            entry = entry | word << (TinyBcamAvmmAddrs.REG_WIDTH_BYTES * 8 * w)
    ...
    def write_table_row(self, row_idx, key, action_id, action_params, timeout_s=0.01):
        ...
        for w in range(self.words_per_entry):
            self.sdr.mmi_write(
                self.offset + TinyBcamAvmmAddrs.CAM_DATA0 + TinyBcamAvmmAddrs.REG_WIDTH_BYTES * w,
