...
class TinyBcamAvmm(AvmmCommonCtrl):
    ...
    def load_config_from_dict(self, dict):
        ...
        for row_idx, entry_dict in enumerate(dict):
            action_params = self._cat_fields(reversed(entry_dict["action_params"]))
            key = self._cat_fields(entry["keys"])
            self.write_table_row(row_idx, key, entry_dict["action_id"]["value"], action_params,)


{'intf_map':
    [
        {
            'key': [{'name': 'ingres_port', 'value': 1, 'bits': 10}],
            'action_id': {'bits': 2, 'value': 0},
            'action_params': [
                {'name': 'vrf_id', 'value': 305419896, 'bits': 32},
                {'name': 'vlan_id', 'value': 100, 'bits': 12}
            ]
        },
        {
            'key': [{'name': 'ingres_port', 'value': 2, 'bits': 10}],
            'action_id': {'bits': 2, 'value': 0},
            'action_params': [
                {'name': 'vrf_id', 'value': 305419896, 'bits': 32},
                {'name': 'vlan_id', 'value': 101, 'bits': 12}
            ]
        }
    ]
}
