...
class TinyBcamAvmm(AvmmCommonCtrl):
    ...
    rd_flag = MMIDescBit(name="rd_flag", addr=TinyBcamAvmmAddrs.CAM_CTRL, bit=TinyBcamAvmmAddrs.CTRL_RD_BIT)
    wr_flag = MMIDescBit(name="wr_flag", addr=TinyBcamAvmmAddrs.CAM_CTRL, bit=TinyBcamAvmmAddrs.CTRL_WR_BIT)
    reset = MMIDescBit(name="reset", addr=TinyBcamAvmmAddrs.CAM_CTRL, bit=TinyBcamAvmmAddrs.CTRL_RST_BIT)
    ...
    def __init__(self, sdr_host, offset, key_bits, action_id_bits, action_param_bits, num_rows):
        ...
        # TinyBcamAvmmAddrs.CAM_DATA0 + 2*words_per_key, where the factor of 2 comes from leaving room for
        # both the key and the mask (which is unused in the BCAM, as opposed to TCAM)
