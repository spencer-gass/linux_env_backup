...
def _get_flow_def_rd_data(regs):
    ...
    remaining_bits_in_reg = 32
    current_reg = 0
    for i, width in enumerate(flow_def.get_field_widths()):
        remaining_bits_in_field = width
        while (remaining_bits_in_field > 0):
            print("remaining_reg {:X}, remaining bits {}, bit ptr {}".format(remaining_reg, remaining_bits_in_field, remaining_bits_in_reg))
            if (remaining_bits_in_field < remaining_bits_in_reg):
                ...
                regs[current_reg] &= (1 << (remaining_bits_in_field - remaining_bits_in_reg)) - 1
                remaining_bits_in_field -= remaining_bits_in_reg
                remaining_bits_in_reg = 32
