...
def _get_flow_def_rd_data(regs):
    ...
    for i, width in enumerate(f.get_field_widths()):
        remaining_bits_in_field = width
        while (remaining_bits_in_field > 0):
            print("field {:X}, reg {:X}, bits in field {}, bits in reg {}".format(fields[i], regs[current_reg], remaining_bits_in_field, remaining_bits_in_reg))
            if (remaining_bits_in_field < remaining_bits_in_reg):
    ...
    f.mac_da, f.mac_sa, f.ether_type, f.vlan_valid, f.vlan_tag, f.num_mpls_labels, f.mpls_label0, f.mpls_label1, \
    f.ip_version, f.ip_ihl, f.ip_dscp, f.ip_ecn, f.ip_length, f.ip_id, f.ip_flags, f.ip_frag_ofs, f.ip_ttl, f.ip_prot, f.ip_hdr_chk, f.ip_sa, f.ip_da, \
    f.pkt_blen_mode, f.pkt_blen_min, f.pkt_blen_ma0x, f.payload_mode, f.payload_value = fields
