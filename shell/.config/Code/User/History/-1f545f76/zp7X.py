...
def _set_flow_def_wr_data(flow_def):
    ...
    for field, width in zip(flow_def.get_fields(), flow_def.get_field_widths()):
        ...
        while (remaining_bits_in_field > 0):
            ...
            if (remaining_bits_in_field < remaining_bits_in_reg+1):
                wdata |= remaining_field << (remaining_bits_in_reg+1 - remaining_bits_in_field)
                remaining_bits_in_reg -= remaining_bits_in_field
                remaining_bits_in_field = 0
            else:
                wdata |= remaining_field >> (remaining_bits_in_field - remaining_bits_in_reg - 1)
                remaining_field &= (1 << (remaining_bits_in_field - remaining_bits_in_reg - 1)) - 1
                remaining_bits_in_field -= (remaining_bits_in_reg + 1)
                remaining_bits_in_reg = 31
