...
def _set_flow_def_wr_data(flow_def):
    ...
    for field, width in zip(flow_def.get_fields(), flow_def.get_field_widths()):
        ...
        while (remaining_bits_in_field > 0):
            print("remaining_field {:X}, remaining bits {}, bit ptr {}".format(remaining_field, remaining_bits_in_field, bit_ptr))
            if (remaining_bits_in_field < bit_ptr+1):
                wdata |= remaining_field << (bit_ptr+1 - remaining_bits_in_field)
                bit_ptr -= remaining_bits_in_field
                remaining_bits_in_field = 0
            else:
                wdata |= remaining_field >> (remaining_bits_in_field - bit_ptr - 1)
                remaining_field &= (1 << (remaining_bits_in_field - bit_ptr - 1)) - 1
                print(" remaining field {:X}".format(remaining_field))
                remaining_bits_in_field -= (bit_ptr + 1)
                bit_ptr = 31
