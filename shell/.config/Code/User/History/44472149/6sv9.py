...
class NetworkPacketGeneratorAvmm(AvmmCommonCtrl):
    ...
    def _set_flow_def_wr_data(self, flow_def):
        ...
        wdata = 0
        bit_ptr = 31
        reg_width_bits = NetworkPacketGeneratorAvmmAddrs.REG_WIDTH_BYTES * 8
        for field, width in zip(flow_def.get_fields(), flow_def.get_field_widths()):
            ...
            while (remaining_bits_in_field > 0):
                if (width < bit_ptr+1):
                    wdata |= field << bit_ptr - width
                    bit_ptr -= width
                    remaining_bits_in_field = 0
                else:
                    wdata |= field >> (width - bit_ptr - 1)
                    bit_ptr = 31
                    remaining_bits_in_field -= (width - bit_ptr - 1)
