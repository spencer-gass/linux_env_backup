...
class NetworkPacketGeneratorAvmm(AvmmCommonCtrl):
    ...
    def _get_flow_def_rd_data(self, regs):

        ### replace regs with register reads to create a regs list

        fd = FlowDef()
        fields = [0] * len(fd.get_fields())
        remaining_bits_in_reg = NetworkPacketGeneratorAvmmAddrs.REG_WIDTH_BITS
        current_reg = 0
        for i, width in enumerate(fd.get_field_widths()):
            remaining_bits_in_field = width
            while (remaining_bits_in_field > 0):
                if (remaining_bits_in_field < remaining_bits_in_reg):
                    ...
                    fields[i] |= ((1 << remaining_bits_in_reg) -1 & regs[current_reg]) << (remaining_bits_in_field - remaining_bits_in_reg)
                    remaining_bits_in_field -= remaining_bits_in_reg
                    remaining_bits_in_reg = NetworkPacketGeneratorAvmmAddrs.REG_WIDTH_BITS
                    current_reg += 1

        fd.mac_da, fd.mac_sa, fd.ether_type, fd.vlan_valid, fd.vlan_tag, fd.num_mpls_labels, fd.mpls_label0, fd.mpls_label1, \
        fd.ip_version, fd.ip_ihl, fd.ip_dscp, fd.ip_ecn, fd.ip_length, fd.ip_id, fd.ip_flags, fd.ip_frag_ofs, fd.ip_ttl, fd.ip_prot, fd.ip_hdr_chk, fd.ip_sa, fd.ip_da, \
        fd.pkt_blen_mode, fd.pkt_blen_min, fd.pkt_blen_ma0x, fd.payload_mode, fd.payload_value = fields
