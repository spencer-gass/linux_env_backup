...
proc ::kepler::vivado::ips::export_ip_info {ip_name args} {
    ...
    set key_in_huddle [string equal [::huddle keys [::huddle get $ip_info config] $key] $key]
    puts "debug: key in huddle: $key_in_huddle"
    puts "debug: [::huddle keys ip_info config $key]"
    puts "debug: [::huddle keys ip_info config ]"
    puts "debug: [::huddle keys ip_info]"
    puts "debug: [string equal "" ""]"
    if {([lrange [split $ipdef :] 0 2] == {xilinx.com ip vitis_net_p4}) && $key_in_huddle} {
        puts "debug: Found P4 IP"
        set value [::huddle get ip_info config $key]
