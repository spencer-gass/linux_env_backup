...
proc ::kepler::vivado::ips::export_ip_info {ip_name args} {
    ...
    set key "CONFIG.P4_FILE"
    set key_in_huddle [lsearch [::huddle keys [::huddle get $ip_info config]] $key]
    puts "debug: key in huddle: $key_in_huddle"
    puts "debug: [::huddle keys [::huddle get $ip_info config]]"
    puts "debug: [lsearch [::huddle keys [::huddle get $ip_info config]] $key]"
    puts "debug: [string equal "" ""]"
    if {([lrange [split $ipdef :] 0 2] == {xilinx.com ip vitis_net_p4}) && $key_in_huddle} {
        puts "debug: Found P4 IP"
        set value [::huddle get [::huddle get $ip_info config] $key]
        if {[file pathtype $value] != "relative"} {
            puts "Warning: p4 file path is absolute but needs to be relative."
            if {[lsearch [file split $value] "p4"] == -1} {
                error "p4 file path does not include kepler/p4 directory"
            }
            set new_path [fileutil::relative $proj_dir $value]
            puts "Warning: Changing p4 path from $value to $new_path"
            ::set_property $key "$new_path" $ip
            ::huddle set ip_info config $key "$new_path"
        }
    }
