...
proc ::kepler::vivado::ips::export_ip_info {ip_name args} {
    ...
    foreach key [::list_property $ip CONFIG.*] {
        ...
        # Verify that p4 path is relative if it exists
        if {$key == "CONFIG.P4_FILE"} {
            puts "Found p4 file path"
            if {[string first "." $value] == 0} {
                puts "p4 file path is relative"
            } else {
                puts "p4 file path is absolute!"
                set p4_index [string first "p4" $value]
                if {$p4_index == -1} {
                    error "p4 file path does not include kepler/p4 directory"
                }
                puts "Changing p4 path from $value to ../../../../[string range $value $p4_index end]"
                ::huddle set ip_info config $key "../../../../[string range $value $p4_index end]"
            }
        }
    }

    # Check if the key exists in the huddle
    # if {[dict exists $huddle $key]} {
    #     set value [dict get $huddle $key]

    #     # Check if it's a relative path
    #     if {[string first "/" $value] != 0 && ![regexp {^[a-zA-Z]:} $value]} {
    #         puts "$key contains a relative path: $value"
    #     } else {
    #         puts "$key contains an absolute path: $value"
    #     }
    # } else {
    #     puts "$key does not exist in the huddle"
    # }
