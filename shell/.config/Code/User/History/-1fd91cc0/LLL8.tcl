...
proc ::kepler::vivado::ips::export_ip_info {ip_name args} {
    ...
    foreach key [::list_property $ip CONFIG.*] {
        ...
        # if {$key == "CONFIG.P4_FILE"} {
        #     if {[string first "." $value] != 0} {
        #         puts "Warning: p4 file path is absolute but needs to be relative."
        #         set p4_dir_index [string first "p4" $value]
        #         if {$p4_dir_index == -1} {
        #             error "p4 file path does not include kepler/p4 directory"
        #         }
        #         puts "Warning: Changing p4 path from $value to ../../../../[string range $value $p4_dir_index end]"
        #         ::huddle set ip_info config $key "../../../../[string range $value $p4_dir_index end]"
        #     }
        # }

        if {([lrange [split $ipdef :] 0 2] == {xilinx.com ip vitis_net_p4}) && ($key == "CONFIG.P4_FILE")} {
            puts "DBG: Found P4 IP."
            if {[file pathtype $value] != "relative"} {
                ...
                puts "DBG: properly formated file path."
            }
        }
