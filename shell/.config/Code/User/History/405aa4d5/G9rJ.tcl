...
proc ::kepler::vivado::generate_ip_sim_target { ip_type } {
    # Generate simulation outputs for all IP cores in the current project of the specified type.
    #
    # ip_type - matched against the second component of the IPDEF property of the IP core.

    foreach ip [::get_ips] {
        set ipdef [::get_property IPDEF $ip]
        if {[lindex [split $ipdef :] 2] == $ip_type} {
            set ip_name [::get_property NAME $ip]
            puts "Generating simulation output products for $ip_name"
            ::generate_target simulation $ip -force
        }
    }
}
