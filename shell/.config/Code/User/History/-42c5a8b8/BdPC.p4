...
/* Enable if testing with BMv2
 * Uses Standard Metadata vs. User Metadata.
 * Disable if testing in production (Xilinx VitisNet).
 */
#define USE_BM 0

#if USE_BM
#include <v1model.p4> // TODO(jjardim): v1model only - remove
#else
#include <xsa.p4>
#endif
...
#if USE_BM
typedef bit<9> PortId;  // TODO(jjardim): v1model only - remove
#else
...
control FRRdplaneIngress(inout headers hdr,
    ...
    action drop() {
#if USE_BM
        mark_to_drop(standard_metadata); // TODO(jjardim): v1model only - remove
#else
    ...
    *************************************************************************/
     ...
     * - Initialize user metadata, namely the VRF and VLAN ID's associated with an FPGA interface.
     ...
     *   sub-interface IP address/route table ID or a ingress FPGA interface/well-known MAC address.
     *   Tag accordingly with associated VLAN ID and forward to PS.
     */
    ...
    /* VLAN ID to FPGA Interface Mapping */
    ...
    /* Interface to CPU sub-interface VLAN and VRF Mapping */
...
#if USE_BM
        standard_metadata.egress_spec = CPU_PORT; // TODO(jjardim): v1model only - remove
#else
    ...
    *************************************************************************/
     ...
     * - Performed at the LER (Label Edge Router): label push (ingressing the MPLS network).
    ...
    /* Forward packet locally/to directly connected interface */
    ...
    /* Push a single-label stack (VPN) to packet at the edge of the MPLS network */
    ...
    /* Push label stack (transport and VPN) to packet at the edge of the MPLS network */
...
#if USE_BM == 0 /* TODO(jjardim): disable BMv2 to test in a production env */
    ...
    apply {
        ...
        if (meta.ingress_port != CPU_PORT) { // non-CPU originated
            ...
            if (hdr.eth.type == TYPE_MPLS) {
                ...
                if (!cmp_ipv4_fib.apply().hit) { /* CMP/Control traffic top priority? */
                    ipv4_fib_ingress.apply();
                }
...
#endif
        /* NOTE(jjardim): original pseudocode :') */
        // if packet is mpls
...
#if USE_BM
V1Switch( // TODO(jjardim): v1model only; remove
FRRdplaneParser(),
