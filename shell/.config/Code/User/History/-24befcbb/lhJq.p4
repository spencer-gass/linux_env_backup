...
typedef bit<9> EgressSpec; // TODO(jjardim): remove
typedef bit<9> PortId; // TODO(jjardim): remove
const PortId CPU_OUT_PORT = 0x1; // TODO(jjardim): remove
...
parser FRRdplaneParser(packet_in packet,
    ...
    state parse_mpls_payload {
        ...
        transition select(hdr.raw_byte.first_byte[7:4]) {
            4: parse_ipv4;
            6: parse_ipv6;
            default: accept;
        }
...
control FRRdplaneIngress(inout headers hdr,
    ...
    /* NOTE(jjardim): can be removed; only used for testing locally */
    action fwd_port(EgressSpec port) {
    ...
    *************************************************************************/
    ...
    action tag_dot1q (VRFId vrf_id, /*IPv4Addr ip_prefix,*/ VlanId vlan_id, /*MacAddr src_addr, MacAddr dst_addr,*/ EgressSpec port) {
        ...
        if (hdr.ipv4.isValid()) {
        // if (meta.ip_hdr_type == 4) {
            hdr.vlan.pcp = hdr.ipv4.tos[7:5];
        } else if (hdr.ipv6.isValid()) {
        // } else if (meta.ip_hdr_type == 6) {
    ...
    *************************************************************************/
    ...
    action mpls_push(VRFId vrf_id, IPv4Addr ip_prefix, MplsLabel out_label, /*LabelStack label_stack, */MacAddr src_addr, MacAddr dst_addr, EgressSpec port) {
        ...
        /* TX (FPGA) interface */
        standard_metadata.egress_spec = port; // TODO(jjardim): update with user metadata port
    }
    ...
    table cmp_egress {
        key = {
            meta.port: exact; // CPU interface ID (incoming)
        }
        actions = {
            strip_dot1q;
            drop;
    ...
    table mpcu_ingress {
        key = {
            meta.port: exact; // FPGA interface ID (incoming)
