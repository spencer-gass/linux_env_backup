...
// typedef bit<20*NUM_LABELS>  MplsLabelStackVec; // Not valid with xilinx compiler
typedef bit<160>  MplsLabelStackVec;
typedef bit<256>  MplsHeaderStackVec;
typedef bit<8>    MplsLabelValidVec;
...
struct user_metadata {
    ...
    VrfId vrf_id;
    bit<1> update_mpls;
    MplsLabelStackVec newMplsStack;
    MplsLabelValidVec newMplsValid;
}
...
struct mpls_update_in_t {
    MplsLabelStackVec new_labels;
    MplsLabelValidVec new_labels_valid;
    MplsLabelStackVec old_labels;
}

struct mpls_update_out_t {
    MplsHeaderStackVec updated_labels;
    MplsLabelValidVec updated_labels_valid;
}
...
control FRRdplaneIngress(inout headers hdr,
    ...
    *************************************************************************/
    ...
    mpls_update_in_t mpls_update_in;
    mpls_update_out_t mpls_update_out;

    UserExtern<ipv4_t,bit<1>>(3) UserIPv4ChkVerify;
    UserExtern<chksum_update_in_t,bit<16>>(1) UserIPv4ChkUpdate;
    UserExtern<mpls_update_in_t,mpls_update_out_t>(4) UserMplsStackUpdate;
    ...
    apply {
        ...
        if (meta.update_mpls == 1) {
            mpls_update_in.old_labels = 0;
            mpls_update_in.old_labels_valid = 0;

            mpls_update_in.new_labels        = meta.newMplsStack;
            mpls_update_in.new_labels_valid  = meta.newMplsValid;

            // Serialize old mpls header
            mpls_update_in.old_labels[7:0] = hdr.mpls[0].ttl;
            mpls_update_in.old_labels[8:8] = hdr.mpls[0].bos;
            mpls_update_in.old_labels[11:9] = hdr.mpls[0].exp;
            mpls_update_in.old_labels[31:12] = hdr.mpls[0].label;
            mpls_update_in.old_labels[39:32] = hdr.mpls[1].ttl;
            mpls_update_in.old_labels[40:40] = hdr.mpls[1].bos;
            mpls_update_in.old_labels[43:41] = hdr.mpls[1].exp;
            mpls_update_in.old_labels[63:44] = hdr.mpls[1].label;
            mpls_update_in.old_labels[71:64] = hdr.mpls[2].ttl;
            mpls_update_in.old_labels[72:72] = hdr.mpls[2].bos;
            mpls_update_in.old_labels[75:73] = hdr.mpls[2].exp;
            mpls_update_in.old_labels[95:76] = hdr.mpls[2].label;
            mpls_update_in.old_labels[103:96] = hdr.mpls[3].ttl;
            mpls_update_in.old_labels[104:104] = hdr.mpls[3].bos;
            mpls_update_in.old_labels[107:105] = hdr.mpls[3].exp;
            mpls_update_in.old_labels[127:108] = hdr.mpls[3].label;
            mpls_update_in.old_labels[135:128] = hdr.mpls[4].ttl;
            mpls_update_in.old_labels[136:136] = hdr.mpls[4].bos;
            mpls_update_in.old_labels[139:137] = hdr.mpls[4].exp;
            mpls_update_in.old_labels[159:140] = hdr.mpls[4].label;
            mpls_update_in.old_labels[167:160] = hdr.mpls[5].ttl;
            mpls_update_in.old_labels[168:168] = hdr.mpls[5].bos;
            mpls_update_in.old_labels[171:169] = hdr.mpls[5].exp;
            mpls_update_in.old_labels[191:172] = hdr.mpls[5].label;
            mpls_update_in.old_labels[199:192] = hdr.mpls[6].ttl;
            mpls_update_in.old_labels[200:200] = hdr.mpls[6].bos;
            mpls_update_in.old_labels[203:201] = hdr.mpls[6].exp;
            mpls_update_in.old_labels[223:204] = hdr.mpls[6].label;
            mpls_update_in.old_labels[231:224] = hdr.mpls[7].ttl;
            mpls_update_in.old_labels[232:232] = hdr.mpls[7].bos;
            mpls_update_in.old_labels[235:233] = hdr.mpls[7].exp;
            mpls_update_in.old_labels[255:236] = hdr.mpls[7].label;

            mpls_update_in.old_labels_valid  = 0;
            if (hdr.mpls[0].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x01;
            }
            if (hdr.mpls[1].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x02;
            }
            if (hdr.mpls[2].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x04;
            }
            if (hdr.mpls[3].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x08;
            }
            if (hdr.mpls[4].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x10;
            }
            if (hdr.mpls[5].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x20;
            }
            if (hdr.mpls[6].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x40;
            }
            if (hdr.mpls[7].isValid()) {
                mpls_update_in.old_labels_valid  = mpls_update_in.old_labels_valid | 0x80;
            }

            UserMplsStackUpdate.apply(mpls_update_in, mpls_update_out);

            hdr.mpls[0].ttl   = mpls_update_out.updated_labels[7:0];
            hdr.mpls[0].bos   = mpls_update_out.updated_labels[8:8];
            hdr.mpls[0].exp   = mpls_update_out.updated_labels[11:9];
            hdr.mpls[0].label = mpls_update_out.updated_labels[31:12];
            hdr.mpls[1].ttl   = mpls_update_out.updated_labels[39:32];
            hdr.mpls[1].bos   = mpls_update_out.updated_labels[40:40];
            hdr.mpls[1].exp   = mpls_update_out.updated_labels[43:41];
            hdr.mpls[1].label = mpls_update_out.updated_labels[63:44];
            hdr.mpls[2].ttl   = mpls_update_out.updated_labels[71:64];
            hdr.mpls[2].bos   = mpls_update_out.updated_labels[72:72];
            hdr.mpls[2].exp   = mpls_update_out.updated_labels[75:73];
            hdr.mpls[2].label = mpls_update_out.updated_labels[95:76];
            hdr.mpls[3].ttl   = mpls_update_out.updated_labels[103:96];
            hdr.mpls[3].bos   = mpls_update_out.updated_labels[104:104];
            hdr.mpls[3].exp   = mpls_update_out.updated_labels[107:105];
            hdr.mpls[3].label = mpls_update_out.updated_labels[127:108];
            hdr.mpls[4].ttl   = mpls_update_out.updated_labels[135:128];
            hdr.mpls[4].bos   = mpls_update_out.updated_labels[136:136];
            hdr.mpls[4].exp   = mpls_update_out.updated_labels[139:137];
            hdr.mpls[4].label = mpls_update_out.updated_labels[159:140];
            hdr.mpls[5].ttl   = mpls_update_out.updated_labels[167:160];
            hdr.mpls[5].bos   = mpls_update_out.updated_labels[168:168];
            hdr.mpls[5].exp   = mpls_update_out.updated_labels[171:169];
            hdr.mpls[5].label = mpls_update_out.updated_labels[191:172];
            hdr.mpls[6].ttl   = mpls_update_out.updated_labels[199:192];
            hdr.mpls[6].bos   = mpls_update_out.updated_labels[200:200];
            hdr.mpls[6].exp   = mpls_update_out.updated_labels[203:201];
            hdr.mpls[6].label = mpls_update_out.updated_labels[223:204];
            hdr.mpls[7].ttl   = mpls_update_out.updated_labels[231:224];
            hdr.mpls[7].bos   = mpls_update_out.updated_labels[232:232];
            hdr.mpls[7].exp   = mpls_update_out.updated_labels[235:233];
            hdr.mpls[7].label = mpls_update_out.updated_labels[255:236];


            if (mpls_update_out.updated_labels_valid & 0x01 == 0x01){
                hdr.mpls[0].setValid();
            } else {
