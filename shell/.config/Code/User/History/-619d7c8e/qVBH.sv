...
#(
    ...
    parameter int                       NUM_MAC_AXIS        = 0,              // number of AXIS interfaces towards MAC 1G/2.5G
    ...
    //TODO(sgass) support other interface types as needed.
    ...
    localparam int  NUM_MP_INTFS = NUM_PHY_GMIIS
                                 + NUM_PHY_XGMIIS
                                 + NUM_PHY_PPLS
                                 + NUM_MAC_GMIIS
                                 + NUM_MAC_XGMIIS
    ...
    `ELAB_CHECK_EQUAL( AXIS_DATA_BYTES, 2**$clog2(AXIS_DATA_BYTES) );
    ...
    logic [NUM_MP_INTFS-1:0] mp_fifo_tx_error_underflow;
    logic [NUM_MP_INTFS-1:0] mp_fifo_tx_fifo_overflow;
    logic [NUM_MP_INTFS-1:0] mp_fifo_tx_fifo_bad_frame;
    logic [NUM_MP_INTFS-1:0] mp_fifo_tx_fifo_good_frame;
    logic [NUM_MP_INTFS-1:0] mp_fifo_rx_error_bad_frame;
    logic [NUM_MP_INTFS-1:0] mp_fifo_rx_error_bad_fcs;
    logic [NUM_MP_INTFS-1:0] mp_fifo_rx_fifo_overflow;
    logic [NUM_MP_INTFS-1:0] mp_fifo_rx_fifo_bad_frame;
    logic [NUM_MP_INTFS-1:0] mp_fifo_rx_fifo_good_frame;

    logic [31:0] avmm_gpio_out [0:NUM_MP_INTFS];
    ...
    AXIS_int #(
        .DATA_BYTES(AXIS_DATA_BYTES)
    ) multipoint_axis_ingress [NUM_MP_INTFS-1:0] (
        .clk     ( int_axis_clk     ),
        .sresetn ( int_axis_sresetn )
    );
    ...
    AXIS_int #(
        .DATA_BYTES(AXIS_DATA_BYTES)
    ) multipoint_axis_egress [NUM_MP_INTFS-1:0] (
        .clk     ( int_axis_clk     ),
        .sresetn ( int_axis_sresetn )
    );
    ...
    assign avmm_gpio_in[0] = NUM_MP_INTFS;

    //TODO(sgass) Add sticky bits or counters for fifo events.

    avmm_gpio #(
        .MODULE_VERSION          ( MODULE_VERSION ),
        .MODULE_ID               ( MODULE_ID      ),
        .DATALEN                 ( 32             ),
        .NUM_INPUT_REGS          ( NUM_MP_INTFS+1 ),
        .NUM_OUTPUT_REGS         ( 1              ),
    ...
    eth_mac_1g_fifo #(
        .AXIS_DATA_WIDTH    ( 8 * AXIS_DATA_BYTES         ),
        .AXIS_KEEP_ENABLE   ( 0                           ),
        .MIN_FRAME_LENGTH   ( 64                          ),
        .ENABLE_PADDING     ( 1                           ),
    ...
    ethernet_to_axis #(
        ...
        .clk_ifc_axis            (                  ),
        .sreset_ifc_axis         (    ),
        .axis_tx                 ( multipoint_axis_egress  ),
        .axis_rx                 ( multipoint_axis_ingress ),

        .fifo_tx_error_underflow ( mp_fifo_tx_error_underflow  [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_tx_fifo_overflow   ( mp_fifo_tx_fifo_overflow    [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_tx_fifo_bad_frame  ( mp_fifo_tx_fifo_bad_frame   [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_tx_fifo_good_frame ( mp_fifo_tx_fifo_good_frame  [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_rx_error_bad_frame ( mp_fifo_rx_error_bad_frame  [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_rx_error_bad_fcs   ( mp_fifo_rx_error_bad_fcs    [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_rx_fifo_overflow   ( mp_fifo_rx_fifo_overflow    [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_rx_fifo_bad_frame  ( mp_fifo_rx_fifo_bad_frame   [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] ),
        .fifo_rx_fifo_good_frame ( mp_fifo_rx_fifo_good_frame  [NUM_MAC_GMIIS + NUM_MAC_XGMIIS - 1 : 0] )
    );


    ////////////////////////////////////////////////////////////////////////////////////////////////
    // SUB-SECTION: AXIS Point to Multi-Point Broadcast Interconnect


    axis_broadcast_wrapper #(
        .N ( N )
    ) point_to_multipoint_broadcast (
        axis_in  ( point_axis_ingress     ),
        axis_out ( multipoint_axis_egress )
    );
    ...
    axis_arb_mux_wrapper #(
        ...
        .axis_in        ( multipoint_axis_ingress ),
        .axis_out       ( point_axis_egress       ),
        .grant          (  ),
        .grant_valid    (  ),
        .grant_encoded  (  )
    );
