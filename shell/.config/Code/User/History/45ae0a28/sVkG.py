...
def create_testbench(vu, lib, project_path, **kwargs):  # pylint: disable=unused-argument
    ...
    sources = [
            ...
            "sim/axis/axis_array_packet_generator_and_checker_tb.sv",
    ]
    for f in sources:
        lib.add_source_files(str(Path(project_path) / f))

    tb = lib.test_bench("axis_array_packet_generator_and_checker_tb")

    test_case.add_config( "test_axis_big_endian", parameters={'AXIS_BIG_ENDIAN': 1})
    test_case.add_config( "test_axis_little_endian", parameters={'AXISBIG_ENDIAN': 0})
