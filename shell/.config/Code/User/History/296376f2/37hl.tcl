...
proc ::kepler::vivado::ips::vnp4_absolute_to_relative_path {ip ipdef proj_dir ip_info property_name} {
    upvar $ip_info ip_info_ref
    puts $property_name
    if {([lrange [split $ipdef :] 0 2] == {xilinx.com ip vitis_net_p4}) && [::list_property $ip $property_name] == $property_name} {
        set existing_path [string trim [::get_property $property_name $ip]]
        puts $existing_path
        if {[file pathtype $existing_path] != "relative"} {
            puts "Warning: $property_name path is absolute but needs to be relative."
            set new_path [fileutil::relative $proj_dir $existing_path]
            puts "Warning: Changing path from $existing_path to $new_path"
            ::set_property $property_name "$new_path" $ip
            ::huddle set ip_info_ref config $property_name "$new_path"
        }
    }
}
...
proc ::kepler::vivado::ips::export_ip_info {ip_name args} {
    ...
    set proj [::current_project]
    set proj_dir [file normalize "[::get_property DIRECTORY $proj]"]
    ...
    puts "Check property paths"
    ::kepler::vivado::ips::vnp4_absolute_to_relative_path $ip $ipdef $proj_dir ip_info "CONFIG.P4_FILE"
    ::kepler::vivado::ips::vnp4_absolute_to_relative_path $ip $ipdef $proj_dir ip_info "CONFIG.JSON_FILE"
    ...
    #         }
    #         set new_path [fileutil::relative $proj_dir $p4_path]
    ...
    # # Verify that the path to a VNP4 json file is relative if it exists
    # set json_path_property "CONFIG.JSON_FILE"
    # if {([lrange [split $ipdef :] 0 2] == {xilinx.com ip vitis_net_p4}) && [::list_property $ip $json_path_property] == $json_path_property} {
    #     set json_path [string trim [::get_property $json_path_property $ip]]
    #     if {[file pathtype $json_path] != "relative"} {
    #         puts "Warning: json file path is absolute but needs to be relative."
    #         set new_path [fileutil::relative $proj_dir $json_path]
    #         puts "Warning: Changing json path from $json_path to $new_path"
    #         ::set_property $json_path_property "$new_path" $ip
    #         ::huddle set ip_info config $json_path_property "$new_path"
    #     }
    # }
...
proc ::kepler::vivado::ips::helpers::fixup_bd_tcl {filepath} {
    ...
    set found_root_design 0
    set found_insert_location 0
    set done_insertions 0
    while {[gets $fin line] >= 0} {
        if {! $found_root_design} {
            if {[string trim $line] == "proc create_root_design \{ parentCell \} \{"} {
                set found_root_design 1
            }
